...
| Scopes | `openid`, `.../auth/userinfo.email`, `.../auth/userinfo.profile` (non-sensitive). For **live-class decks**, add `https://www.googleapis.com/auth/presentations` and `https://www.googleapis.com/auth/drive` (sensitive — Testing + test users is enough). `drive.file` cannot copy Lesson Theme Template #1 because this app did not create that file. Operators who connected earlier must **Connect Google Slides** again after this scope change. Enable **Google Slides API** and **Google Drive API**. |
