...
Volume `/data` is `LLOVES_DB=/data/lloves.sqlite` plus unpacked IMSCC. Fly volumes are **encrypted at rest by default** (do not recreate `lloves_data` with `--no-encryption`). TLS in transit is Fly `force_https` plus the `alc.mckenzian.com` certificate. The app does **not** use SQLCipher; disk/volume encryption is the at-rest control. Secrets stay in Fly secrets / `lms/.env` — never in client JS. Do not deploy from `lms/` — the image would miss curriculum PDFs and semester.json. The Fly hostname `https://lloves-lms.fly.dev` is for health checks and DNS CNAME targets only; do not register it as the production Google redirect.
