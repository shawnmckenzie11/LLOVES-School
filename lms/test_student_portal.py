...
class StudentPortalTests(unittest.TestCase):
    ...
    def test_beat19_end_class_saves_and_quit_does_not(self) -> None:
        """Beat 19: End Class writes A&P; Quit wipes only; Meet taps skip."""
        from meet_team import meet_team_prompt_payload

        self._join_maple_home()
        maple = self._maple_row()
        maple_id = int(maple["id"])
        state = self.student.get("/api/student/state").get_json()
        prompt = state["prompt"]
        ...
        meet_submit = self.student.post(
            "/api/student/live-prompt/response",
            json={
                "prompt_id": meet_prompt["id"],
                "response": {"choice": "A"},
            },
        )
        self.assertEqual(meet_submit.status_code, 200, meet_submit.get_json())
        self.staff.post(
            f"/api/classes/{self.class_id}/begin",
        ...
        marks = self.school.game.attendance_score_rows(self.class_id)
        ended_sessions = [row for row in marks["sessions"] if row["status"] == "ended"]
        self.assertTrue(ended_sessions)
        session_id = int(ended_sessions[-1]["id"])
        present = next(
            row
            for row in marks["scores"]
            if int(row["session_id"]) == session_id and int(row["student_id"]) == maple_id
        )
        self.assertTrue(present["present"])
        with self.school.game._lock:
            pts = self.school.game.conn.execute(
                """
        ...
        self.assertEqual(float(pts["points_r1"]), 1.0)
        self.assertEqual(float(pts["points_r2"]), 0.0)
        self.assertEqual(float(pts["points_r3"]), 0.0)
        self.assertEqual(float(pts["points"]), 1.0)
        att_page = self.staff.get(
            f"/staff/class/{self.class_id}?tab=ap&view=attendance"
        )
        self.assertEqual(att_page.status_code, 200)
        part = self.staff.get(
            f"/api/classes/{self.class_id}/participation-grid"
        ).get_json()
        self.assertTrue(part.get("ok"))
        cell = part["cells"].get(f"{maple_id}:2026-09-09")
        self.assertIsNotNone(cell)
        self.assertEqual(float(cell["points_r1"]), 1.0)
        self.assertEqual(float(cell["points"]), 1.0)

    def test_beat19_quit_wipes_without_class_column(self) -> None:
        """Beat 19: Quit confirm path writes nothing to A&P."""
        self._join_maple_home()
        state = self.student.get("/api/student/state").get_json()
        self.student.post(
            "/api/student/live-prompt/response",
        ...
        quit_res = self.staff.post(
            f"/staff/class/{self.class_id}/end-live",
            data={"save": "0"},
            follow_redirects=False,
        )
        self.assertEqual(quit_res.status_code, 302)
        loc = quit_res.headers.get("Location", "")
        self.assertTrue(loc.endswith("/staff") or loc.rstrip("/").endswith("/staff"))
        self.assertEqual(self.school.list_live_sessions_for_class(self.class_id), [])
        marks = self.school.game.attendance_score_rows(self.class_id)
        ended = [row for row in marks["sessions"] if row["status"] == "ended"]
